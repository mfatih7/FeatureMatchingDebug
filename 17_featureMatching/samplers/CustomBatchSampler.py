...
class CustomBatchSampler(BatchSampler):
    def __init__(self, n_image_pairs, train_val_test, input_type, N_images_in_batch, N, batch_size ):
...
def get_sampler( dataset, input_type, N_images_in_batch, N, batch_size ):
    
    n_image_pairs = np.int64( len( dataset ) )
    train_val_test = dataset.train_val_test
    
    return CustomBatchSampler( n_image_pairs, train_val_test, input_type, N_images_in_batch, N, batch_size )

if __name__ == '__main__':
    # customBatchSampler = CustomBatchSampler( 1000, 'train', '1_to_1', 2, 2048, 1024 )
    customBatchSampler = CustomBatchSampler( 1000, 'train', '1_to_1', 1, 512, 512 )
