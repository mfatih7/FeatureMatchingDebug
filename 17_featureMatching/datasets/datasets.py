...
class CorrespondencesDataset(Dataset):
    def __init__(self, config, input_type, N_images_in_batch, N, batch_size, train_val_test, chunk):
        ...
        if( self.config.use_hdf5_or_picle=='hdf5' ):
            ...
            if(self.train_val_test != 'val' or (self.train_val_test == 'val' and self.config.validation_chunk_or_all == 'chunk') ):
            ...
            if(self.train_val_test != 'val'):
                data_chunk = self.get_data_for_chunk_from_pickle( chunk )
            # For Validation use all
            else:
                for c in range(config.n_chunks):
                    tmp = self.get_data_for_chunk_from_pickle( c )
                    if(c==0):
                        data_chunk = tmp
                    else:
...
# For TPU operation, all data related to an image pair pumped within the same batch                
    ...
    def get_data_for_chunk_from_hdf5( self, indices_chunk):
        ...
        if(self.config.storage_local_or_bucket=='local'):
            input_path_local_or_bucket = self.config.input_path_local
        elif(self.config.storage_local_or_bucket=='bucket'):
...
def get_dataset( config, input_type, N_images_in_batch, N, batch_size, train_val_test, chunk ):    
    return CorrespondencesDataset(config, input_type, N_images_in_batch, N, batch_size, train_val_test, chunk)
...
if __name__ == '__main__':
    ...
    dataset = CorrespondencesDataset(config, input_type, N_images_in_batch, N, batch_size, 'train', chunk)
    
### GET DATA WTIH DIRECTLY FROM DATASET ######################################################################
...
### GET DATA WTIH SAMPLER AND DATALOADER ######################################################################

    from samplers.CustomBatchSampler import get_sampler
    
    sampler = get_sampler( dataset, input_type, N_images_in_batch, N, batch_size )
